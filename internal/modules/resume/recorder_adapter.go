...
// recorderAdapter bridges the resume application Recorder port to the shared
// sync TxRecorder without leaking sync wire types into the service.
...
func (a recorderAdapter) Record(ctx context.Context, tx pgx.Tx, change application.SyncChange) error {
	...
	return a.recorder.Record(ctx, tx, syncmod.Change{
		...
		EntityType:    syncmod.EntityTypeResume,

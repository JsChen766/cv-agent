...
import type { LLMChatResponse, LLMUsage, ToolCall } from "../agent-core/model/types.js";
...
export async function parseJsonResponse(response: Response, providerName: string): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    throw new AgentError("MODEL_FAILED", `${providerName} request failed (${response.status}): ${message}`, {
      statusCode: response.status,
      cause: body,
      details: { retryable: RETRYABLE_STATUS_CODES.has(response.status) },
    });
  }
...
export function normalizeOpenAIChatResponse(raw: unknown): LLMChatResponse {
  ...
  const usage: LLMUsage | undefined = Object.keys(usageRaw).length
    ? {

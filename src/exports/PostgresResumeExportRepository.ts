...
export class PostgresResumeExportRepository implements ResumeExportRepository {
  ...
  public async updateExport(userId: string, id: string, patch: Partial<ResumeExport>): Promise<ResumeExport | null> {
    await this.database.query(
      `UPDATE resume_export SET job_id=COALESCE($3,job_id), status=COALESCE($4,status), file_id=COALESCE($5,file_id), download_token_hash=COALESCE($6,download_token_hash), download_expires_at=COALESCE($7,download_expires_at), error_message=$8, completed_at=COALESCE($9,completed_at), fit_report=COALESCE($10::jsonb,fit_report), updated_at=$11 WHERE user_id=$1 AND id=$2`,
      [userId, id, patch.jobId ?? null, patch.status ?? null, patch.fileId ?? null, patch.downloadTokenHash ?? null, patch.downloadExpiresAt ?? null, patch.errorMessage ?? null, patch.completedAt ?? null, patch.fitReport ? JSON.stringify(patch.fitReport) : null, new Date().toISOString()],
    );
...
function toExport(row: any): ResumeExport {
  ...
  const fitReport = parseFitReport(row.fit_report);
  if (fitReport) base.fitReport = fitReport;
  return base;
}

function parseFitReport(raw: unknown): ResumeFitReport | undefined {
  if (raw == null) return undefined;
  if (typeof raw === "string") {
    try {
      return JSON.parse(raw) as ResumeFitReport;
    } catch {
      return undefined;
    }
  }
  if (typeof raw === "object") return raw as ResumeFitReport;
  return undefined;
}

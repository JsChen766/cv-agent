...
def _find_section(
    structured: dict[str, Any], section_id: str
) -> dict[str, Any]:
...
def _find_item(
    structured: dict[str, Any], item_id: str
...
def _op_replace_bullet(
    structured: dict[str, Any], op: dict[str, Any]
) -> dict[str, Any]:
...
def _op_add_bullet(
    ...
    if after_bullet_id is None:
        ...
        idx = next(
            (i for i, b in enumerate(bullets) if b.get("id") == after_bullet_id), None
        )
        if idx is None:
...
_ITEM_ALLOWED_FIELDS = frozenset(
    {"title", "organization", "role", "start_date", "end_date", "raw_text"}
)


def _op_replace_item_field(
    structured: dict[str, Any], op: dict[str, Any]
) -> dict[str, Any]:
    item_id = op["item_id"]
    field = op["field"]
...
def _op_add_item(
    ...
    new_item["id"] = _new_item_id()
    if after_item_id is None:
        items.append(new_item)
    else:
        idx = next(
            (i for i, it in enumerate(items) if it.get("id") == after_item_id), None
        )
        if idx is None:

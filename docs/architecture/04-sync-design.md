...
同一 `operationId` 的幂等重放必须保留首次结果的完整恢复语义：除状态、版本与错误码外，
还要原样返回首次结果中的 `serverEntity`。尤其是版本冲突不能在重放时把“服务端实体存在但
版本已变化”退化成空实体；`serverEntity=null` 只表示契约中明确的服务端墓碑/无快照结果，
客户端不得因此把一次普通冲突误判为服务端删除。
...
- 创建时可携带实际投递 Resume 的本地 `contentHash` 快照；普通更新保持不变，显式换绑 Resume
  时由客户端提交新指纹并与标题快照一同刷新；
- 冲突时拉取服务端最新值；
- 非重叠字段可以在本地生成合并预览；

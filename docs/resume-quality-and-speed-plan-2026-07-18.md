...
**状态**：P0–P2 工程与 PostgreSQL 实测完成；P3 工程完成但真实模型局部修复成功率验收未通过；structured 协议缓存、按经历并行 A/B 路径和稀疏模板后端契约已完成；真实模型复验与 P4 DOM/前端模板仍待完成
**日期**：2026-07-18  
...
### P3/P5 与稀疏模板后续工程记录（2026-07-18）

本轮已完成但尚未直接开启生产开关的改动：

- OpenAI-format Provider 记忆最近一次成功的 structured output 协议；首次兼容探测后，后续调用直接走已验证模式，缓存模式失败时仍会执行剩余 fallback 并刷新缓存；
- P3 `ResumeBulletRepairService` 增加逐候选诊断，记录不含正文的 fact、coverage、数字、句号、排版等拒绝码，以及实际尾行比例和最终选中状态；Graph trace 记录候选通过/拒绝数量和拒绝码；
- 新增按经历并行生成的 A/B 路径，以经历为并发粒度，教育、联系方式、skills 和最终单 variant 组装保持确定性；并发上限默认 `3`，任一分支异常或返回越权 fact/JD ID 时回退到原整份生成；
- 新增 `resume-sparse-v1` 版式，正文/标题字号、边距和垂直间距更大；当标准模板最大可用高度低于 `72%` 时可确定性选择稀疏模板，稀疏版密度区间为 `52%–90%`、目标 `68%`；
- 新增前后端共享 manifest `contracts/resume-layout-templates-v1.json` 及同步检查脚本，后端不解析 HTML，浏览器 DOM 仍为分页与换行最终事实；
- `RESUME_PARALLEL_GENERATION_ENABLED` 和 `RESUME_SPARSE_TEMPLATE_ENABLED` 默认关闭，前者等待真实 provider A/B，后者等待前端支持相同 template/profile/hash 后开启。

工程验证：unit suite `315 passed`；本轮变更范围 Ruff 与核心 source mypy 通过。上述结果不改变 P3 的真实模型效果状态：必须利用新增拒绝码重新跑固定真实样例并达到成功率门槛后，P3 才能标记完成。

...
| `app/api/deps.py` | 修改 | 在 `build_service_container` 注入 `ResumeObservabilityService(PostgresResumeObservabilityRepository(pool))`；增加供 DOM observation endpoint 使用的 dependency。依赖装配仍只发生在 API wiring 层。 |
...
| `app/api/routes/product/resume.py` | 修改 | 新增 `POST /v1/product/resumes/{resume_id}/variants/{variant_id}/layout-observations`、严格 camelCase request schema 和序列化；只做 HTTP 校验并调用 domain service，不在 API 层计算业务结论。P0 endpoint 只观测，不改变 variant 状态。 |
...
| `tests/unit/test_api_resume_layout_observations.py` | endpoint camelCase 契约、严格 extra forbid、404/ownership、201/幂等响应；不会改变 variant quality 状态。 |
...
5. 增加 DOM observation endpoint，并用本次问题数据保存第一条 observation；

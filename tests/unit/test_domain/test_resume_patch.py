        ...
        [
            {
                "op": "replace_bullet",
                "bullet_id": "bul-002",
                "text": "Optimized 500k-row SQL scripts",
            }
        ],
    )
...
def test_replace_bullet_id_stability() -> None:
    ...
    result = apply_patch_operations(
        s, [{"op": "replace_bullet", "bullet_id": "bul-001", "text": "Changed text"}]
    )
...
def test_replace_bullet_unknown_id_raises() -> None:
    ...
    with pytest.raises(ValueError, match="bullet_id not found"):
        apply_patch_operations(
            s, [{"op": "replace_bullet", "bullet_id": "bul-nonexistent", "text": "x"}]
        )
...
def test_delete_bullet_happy_path() -> None:
    s = _make_structured()
    result = apply_patch_operations(s, [{"op": "delete_bullet", "bullet_id": "bul-002"}])
    bullets = result["sections"][0]["items"][0]["bullets"]
...
def test_add_bullet_at_end() -> None:
    s = _make_structured()
    result = apply_patch_operations(
        s, [{"op": "add_bullet", "item_id": "item-001", "text": "New bullet"}]
    )
    bullets = result["sections"][0]["items"][0]["bullets"]
...
def test_replace_item_field_unknown_item_raises() -> None:
    ...
    with pytest.raises(ValueError, match="item_id not found"):
        apply_patch_operations(
            s, [{"op": "replace_item_field", "item_id": "item-zzz", "field": "title", "value": "x"}]
        )
...
def test_add_item_preserves_original_ids() -> None:
    ...
    original_ids = _all_ids(s)
    result = apply_patch_operations(
        s, [{"op": "add_item", "section_id": "sec-001", "item": {"title": "New"}}]
    )
...
def test_reorder_items_happy_path() -> None:
    s = _make_structured()
    result = apply_patch_operations(
        s, [{"op": "reorder_items", "section_id": "sec-001", "item_ids": ["item-002", "item-001"]}]
    )
    items = result["sections"][0]["items"]
...
def test_reorder_items_mismatch_raises() -> None:
    ...
    with pytest.raises(ValueError, match="reorder_items id mismatch"):
        apply_patch_operations(
            s, [{"op": "reorder_items", "section_id": "sec-001", "item_ids": ["item-001"]}]
        )
...
def test_reorder_sections_happy_path() -> None:
    s = _make_structured()
    result = apply_patch_operations(
        s, [{"op": "reorder_sections", "section_ids": ["sec-002", "sec-001"]}]
    )
...
def test_reorder_sections_mismatch_raises() -> None:
    ...
    with pytest.raises(ValueError, match="reorder_sections id mismatch"):
        apply_patch_operations(s, [{"op": "reorder_sections", "section_ids": ["sec-001"]}])
...
def test_id_stability_after_replace_bullet() -> None:
    ...
    result = apply_patch_operations(
        s, [{"op": "replace_bullet", "bullet_id": "bul-001", "text": "Updated"}]
    )
...
async def test_service_patch_variant_delegates_to_repo() -> None:
    ...
    svc = ResumeService(repo, ResumeLayoutService(PillowFontMetrics()))
    ...
    assert result.layout_report.status == "profile_mismatch"
    ...
    saved_structured = call_kwargs.kwargs["structured"]
    saved_bullet = saved_structured["sections"][0]["items"][0]["bullets"][0]
    assert saved_bullet["text"] == "Changed"
    assert "layout_usage_ratio" in saved_structured
